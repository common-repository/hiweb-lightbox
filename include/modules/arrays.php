<?php
	...
	class hw_lightbox_arrays {
		 ...
		 * @return null
		 */
		public function get_index( $array, $index = 0 ) {
			if ( ! is_array( $array ) ) {
				return null;
			}
			...
			if ( $this->count( $index ) == 0 ) {
				return $this->get_byKey( array_values( $array ), $indexThis );
			} else {
				return $this->get_byKey( array_values( $this->get_index( $array, $indexThis ) ), $index );
			}
		}
		 ...
		 * @return bool
		 */
		public function is_empty( $mix, $nullVal = true ) {
		...
		public function get_byKey( $haystack = array(), $keyMix = '', $def = null ) {
			...
			if ( is_array( $keyMix ) && count( $keyMix ) > 1 ) {
				...
				return $this->get_byKey( $this->get_byKey( $haystack, $key, $def ), $keyMix, $def );
		 ...
		 * @return array
		 */
		public function get( $mix, $subKey = null ) {
		...
		public function count( $haystack, $keyMix = null ) {
			if ( ! is_null( $keyMix ) ) {
				$haystack = $this->get_byKey( $haystack, $keyMix );
			}
			...
			if ( is_array( $array ) ) {
				foreach ( $array as $key => $value ) {
					if ( $level < 1 ) {
						...
						$R[ $key ] = $this->ungroup( $value, $level - 1 );
					}
				}
			} else {
				$R = '!';
